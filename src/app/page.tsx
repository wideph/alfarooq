        ...
        {settings.heroText && (
          <section className={`relative overflow-hidden ${getHeroPadding(settings.heroText)}`}>
            ...
            <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
              <p
                className={`text-center font-semibold text-slate-900 leading-relaxed urdu-text animate-fade-in ${getHeroTextSize(settings.heroText)}`}
              >
                {settings.heroText}
              </p>
            </div>
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20 pt-8">
          ...
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[1, 2, 3].map((i) => (
                <div key={i} className="rounded-2xl min-h-[22rem] skeleton" />
              ))}
            </div>
          ) : courses.length === 0 ? (
